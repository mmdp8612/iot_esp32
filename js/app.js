...
async function cargarUltimaMedicion() {
    try {
        ...
        document.getElementById('temperatura').textContent = parseFloat(data.temperatura).toFixed(1);
        document.getElementById('humedad').textContent = parseFloat(data.humedad).toFixed(0);
        document.getElementById('deviceId').textContent = data.device_id;
        document.getElementById('ultimaLectura').textContent = formatearFecha(data.fecha);
...
function cargarTabla(datos) {
    ...
    datosInvertidos.forEach(item => {
        ...
        fila.innerHTML = `
            ...
            <td class="px-6 py-4 font-medium">${item.device_id}</td>
            <td class="px-6 py-4">${parseFloat(item.temperatura).toFixed(1)} °C</td>
            <td class="px-6 py-4">${parseFloat(item.humedad).toFixed(0)} %</td>
            <td class="px-6 py-4 text-slate-500">${formatearFecha(item.fecha)}</td>
        `;
...
function actualizarGraficos(datos) {
    ...
    const temperaturas = datos.map(item => parseFloat(item.temperatura));
    const humedades = datos.map(item => parseFloat(item.humedad));
...
function actualizarEstadoDispositivo(fechaUltimaLectura) {
    const fecha = new Date(fechaUltimaLectura.replace(' ', 'T'));
